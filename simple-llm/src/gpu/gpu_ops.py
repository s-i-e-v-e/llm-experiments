...
def __bind_group_entries_translate(entries: List[BindGroupEntry]) -> List[Dict]:
...
def __uniform_buffer_create(
    ...
    """
    return ctx.device.create_buffer_with_data(data=data, usage=wgpu.BufferUsage.UNIFORM)


def __uniform_buffer_create_and_retain(
    ctx: GPUContext, data: np.ndarray
) -> wgpu.GPUBuffer:
    ...
    ctx.batch_state.retained_buffers.append(buffer)
    return buffer


def __bind_group_create(
    ctx: GPUContext,
    ...
    Returns:
        New WGPU bind group
    """
    return ctx.device.create_bind_group(
        layout=pipeline.get_bind_group_layout(0),
        entries=__bind_group_entries_translate(entries),
    )


def batch_begin(ctx: GPUContext, enable_profiling: bool = False) -> None:
    """
    ...
    in batch_state.retained_buffers to keep them alive until batch_commit is called.
...
def batch_add(
    ...
    max_ops = ctx.config.max_batch_operations

    if ctx.batch_state.operation_count >= max_ops:
        raise RuntimeError(
            f"Batch operation limit ({max_ops}) exceeded. "
            f"Call batch_commit() to flush operations."
        )

    # Validate workgroup counts
    max_workgroups = ctx.config.max_workgroups_per_dim
    ...
    params_buffer = __uniform_buffer_create_and_retain(ctx, params)
    pipeline = pipeline_get_or_create(ctx, kernel_code)
    ...
    bindgroup = __bind_group_create(ctx, pipeline, entries)

    assert ctx.batch_state.encoder is not None
    compute_pass = ctx.batch_state.encoder.begin_compute_pass()
    compute_pass.set_pipeline(pipeline)
    compute_pass.set_bind_group(0, bindgroup)
    compute_pass.dispatch_workgroups(workgroups_x, workgroups_y, workgroups_z)
    compute_pass.end()

    ctx.batch_state.operation_count += 1


def batch_copy_add(ctx: GPUContext, source: GPUBuffer2D, dest: GPUBuffer2D) -> None:
    ...
    if ctx.batch_state.encoder is None:
        raise RuntimeError("Must call batch_begin before adding operations")

    ctx.batch_state.encoder.copy_buffer_to_buffer(
        source.buffer, 0, dest.buffer, 0, source.size * 4
    )
    ctx.batch_state.operation_count += 1


def batch_commit(ctx: GPUContext) -> None:
    ...
    command_buffer = ctx.batch_state.encoder.finish()
    ctx.device.queue.submit([command_buffer])

    # Clear encoder and buffers to prevent reuse
    ctx.batch_state.encoder = None
    ctx.batch_state.retained_buffers.clear()
